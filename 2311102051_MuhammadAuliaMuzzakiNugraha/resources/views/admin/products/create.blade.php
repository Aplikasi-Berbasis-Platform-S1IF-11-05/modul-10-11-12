...
@section('content')
    <section class="surface-panel rounded-xl p-5 sm:p-6">
        <div class="mb-6">
            <h1 class="brand-title text-xl font-bold text-slate-100">Tambah Produk Baru</h1>
            <p class="text-sm text-muted">Lengkapi informasi produk untuk ditampilkan pada landing page.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                >
                <p class="mt-2 text-xs text-muted">Jika diisi, sistem akan membuat kategori baru secara otomatis dan langsung memilihnya.</p>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="slug" name="slug" type="text" class="input-field" value="{{ old('slug') }}" required placeholder="contoh-nama-produk">
                    @error('slug')
                        <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" class="textarea-field" required>{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <input id="price" name="price" type="number" min="0" step="0.01" class="input-field" value="{{ old('price') }}" required>
                    @error('price')
                        <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="image" class="mb-2 block text-sm font-semibold text-slate-200">Gambar Produk</label>
                    <input id="image" name="image" type="file" accept="image/png,image/jpeg,image/webp" class="input-field" required>
                    <p class="mt-2 text-xs text-muted">Format: JPG, JPEG, PNG, WEBP (maks 2MB)</p>
                    @error('image')
                        <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex flex-wrap gap-3 pt-2">
                <button type="submit" class="btn-crimson">Simpan Produk</button>

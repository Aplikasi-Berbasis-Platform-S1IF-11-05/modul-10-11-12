    ...
    <main class="flex-1">
        <section class="relative overflow-hidden bg-gradient-to-br from-amber-600 via-amber-700 to-orange-900 text-white">
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    <a href="{{ route('products.index') }}" class="inline-flex items-center rounded-lg bg-white px-5 py-2.5 text-amber-900 font-semibold shadow hover:bg-amber-50">Lihat katalog</a>
                    <a href="{{ route('about') }}" class="inline-flex items-center rounded-lg border border-white/40 px-5 py-2.5 font-semibold hover:bg-white/10">Cerita festival</a>
                </div>
            </div>
        </section>
        ...
        <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                ...
                <a href="{{ route('products.index') }}" class="text-amber-800 font-semibold hover:underline">Semua produk →</a>
            </div>
                        ...
                        <a href="{{ route('products.show', $product) }}" class="group rounded-2xl border border-amber-100 bg-white shadow-sm overflow-hidden hover:shadow-md transition">
                            <div class="aspect-[4/3] bg-amber-100 relative">
                                @if($product->image_url)
                                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="h-full w-full object-cover group-hover:scale-[1.02] transition" loading="lazy">
                                @else
                            ...
                            <div class="p-4">
                                ...
                                <h3 class="mt-1 font-semibold text-stone-900 group-hover:text-amber-800">{{ $product->name }}</h3>
